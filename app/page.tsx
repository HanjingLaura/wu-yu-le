...
function ShelfView({ items, onOpen, onAdd }: { items: ShelfObject[]; onOpen: (story: Story) => void; onAdd: () => void }) {
  ...
  return <section className="view shelf-view"><div className="shelf-heading"><div><p className="eyebrow">2024</p><h1>收藏</h1></div></div><div className="shelf-note"><span>{String(items.length).padStart(2, "0")}</span><span>件记录</span></div><div className="shelf-rack">{rows.map((row, index) => <div className={`shelf-row ${index === 0 ? "shelf-row-top" : "shelf-row-bottom"}`} key={row.map((item) => item.id).join("-")}>{row.map((item) => <ShelfItem key={item.id} item={item} onOpen={onOpen}/>)}{row.length === 1 && <button className="shelf-empty" onClick={onAdd} aria-label="添加物品"><Plus size={19}/></button>}</div>)}{items.length === 0 && <div className="shelf-row shelf-row-bottom"><button className="shelf-empty" onClick={onAdd} aria-label="添加物品"><Plus size={19}/><span>添加一件</span></button></div>}</div></section>;
}

function ShelfItem({ item, onOpen }: { item: ShelfObject; onOpen: (story: Story) => void }) { const story = item.storyId ? stories.find((entry) => entry.id === item.storyId) : undefined; return <button className="shelf-item" onClick={() => story && onOpen(story)} disabled={!story}><span className={`shelf-item-image ${item.cutout ? "is-cutout" : ""}`}><img src={item.image} alt=""/>{item.cutout && <span className="cutout-badge">透明 PNG</span>}</span><span className="shelf-item-copy"><small>{item.date.split(" / ").slice(1).join(".")}</small><strong>{item.title}</strong><em>{item.people.length ? item.people.join(" · ") : "独自"}</em></span></button>; }

function TimelineView({ onOpen, onBack }: { onOpen: (story: Story) => void; onBack: () => void }) { return <section className="view timeline-view"><div className="view-heading"><div><p className="eyebrow">2024</p><h1>史书</h1></div><button className="text-button" onClick={onBack}><ArrowLeft size={16}/> 架子</button></div><div className="timeline">{stories.map((story, index) => <div className="timeline-row" key={story.id}><div className="date-rail"><strong>{story.date.split(" / ").slice(1).join("/")}</strong><span>{story.day}</span>{index !== stories.length - 1 && <i/>}</div><button className={`story-card ${story.tone}`} onClick={() => onOpen(story)}><div className="card-copy"><span className="card-kicker">{story.public ? "公开" : "私藏"}</span><h2>{story.title}</h2><p>{story.excerpt}</p><span className="read-link">打开 <ChevronRight size={15}/></span></div><img src={story.image} alt=""/></button></div>)}</div></section>; }
