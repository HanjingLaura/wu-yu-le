...
function TimelineView({ stories, onOpen, onBack }: { stories: Story[]; onOpen: (story: Story) => void; onBack: () => void }) {
  ...
  const year = orderedStories[0]?.date.split(" / ")[0] ?? "";
  return <section className="view timeline-view"><div className="view-heading"><div>{year ? <p className="eyebrow">{year}</p> : null}</div><button className="icon-button" onClick={onBack} aria-label="返回"><ArrowLeft size={18}/></button></div><div className="timeline">{orderedStories.map((story, index) => <div className="timeline-row" key={story.id}><div className="date-rail"><strong>{story.date.split(" / ").slice(1).join("/")}</strong><span>{story.day}</span>{index !== orderedStories.length - 1 && <i/>}</div><button className={`story-card ${story.tone}`} onClick={() => onOpen(story)}><div className="card-copy"><span className="card-kicker">{story.public ? "公开" : "私藏"}</span><h2>{story.title}</h2><p>{story.excerpt}</p></div>{story.storyImages[0] ? <img src={story.storyImages[0]} alt=""/> : <span aria-hidden="true"/>}</button></div>)}</div></section>;
}
    ...
    <div className="upload-zone">
      <div className="upload-box">
        ...
        <div className="upload-actions">
          <button type="button" onClick={() => cameraRef.current?.click()} aria-label={`拍照，${label}`}>
            拍照
          </button>
          <button type="button" onClick={() => galleryRef.current?.click()} aria-label={`从相册选择，${label}`}>
            从相册选择
          </button>
        </div>
      </div>
