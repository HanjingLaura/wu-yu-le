...
function Brand({ onBook }: { onBook: () => void }) {
  return <div className="brand-mark"><button className="book-launch" onClick={onBook} aria-label="打开时间线"><BookOpen size={23} strokeWidth={1.8} aria-hidden="true" /></button></div>;
}
...
function TimelineView({ onOpen, onBack }: { onOpen: (story: Story) => void; onBack: () => void }) { return <section className="view timeline-view"><div className="view-heading"><div><p className="eyebrow">2024</p><h1>时间线</h1></div><button className="icon-button" onClick={onBack} aria-label="返回"><ArrowLeft size={18}/></button></div><div className="timeline">{stories.map((story, index) => <div className="timeline-row" key={story.id}><div className="date-rail"><strong>{story.date.split(" / ").slice(1).join("/")}</strong><span>{story.day}</span>{index !== stories.length - 1 && <i/>}</div><button className={`story-card ${story.tone}`} onClick={() => onOpen(story)}><div className="card-copy"><span className="card-kicker">{story.public ? "公开" : "私藏"}</span><h2>{story.title}</h2><p>{story.excerpt}</p><span className="read-link">打开 <ChevronRight size={15}/></span></div><img src={story.image} alt=""/></button></div>)}</div></section>; }
